...
class Player(EntityObstacle):
    ...
    def update(self):
        ...
        if pygame.sprite.spritecollide(self, walls, False) or \
            pygame.sprite.spritecollide(self, ice_blocks, False) or \
            pygame.sprite.spritecollide(self, enemies, False) or \
            pygame.sprite.spritecollide(self, iron_walls, False):
            self.rect.x -= self.direction.x * PLAYER_SPEED
    ...
    def shoot(self):
        ...
        if now - self.last_shot_time >= 1200:
            ...
            if self.current_direction == "up":
                ...
                direction.x = 1
            bullet = Bullet(self.rect.center, direction, player_bullets, all_sprites)
            player_bullets.add(bullet)  # додаємо кулю до групи куль
